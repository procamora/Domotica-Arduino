...
/**
 ...
 @param chat_id String id del usuario al que manda un mensaje
 */
void set_rele(String accion, String chat_id, String msgOpcional, boolean apagarModoAutomatico) {
    if (MODO_DEBUG) {
...
void check_timer() {
    ...
    if (global_timer.activo && global_timer.timer != 0) {
        ...
        if (get_timer()[0] == '-') {  //si el numero es negativo salta, como mucho se deberia tratarsar 1.5seg
            ...
            set_rele(global_timer.accion_timer, String(ID_TELEGRAM), "", true);  //ejecuto accion en rele
            global_timer.activo = false;
        ...
        if (global_timer.contador_max == CONTADOR_MAX) {
            ...
            if (get_rele()) {  //si esta encendido compruebo si supero la temperatura maxima
                if (temp > global_timer.temperatura_max) {
                    ...
                    if (MODO_DEBUG)
                        set_rele("off", String(ID_TELEGRAM), msg, false);  //ejecuto accion en rele
                    else
                        //Sino es modo debug solo acciono el rele sin mandar telegram
            ...
            if (temp < global_timer.temperatura_max - 1.5) {
                ...
                String msg = "Automatico: temperatura actual: " + String(temp) + "*C\nEncender";
                if (MODO_DEBUG)
                    set_rele("on", String(ID_TELEGRAM), msg, false);  //ejecuto accion en rele
                else
    ...
    for (int i = 0; i < numNewMessages; i++) {
        ...
        if (String(ID_TELEGRAM) == chat_id) {
            if (parse_comandos[0] == "/exit")
                ...
                set_rele(parse_comandos[1], chat_id, "", true);
        ...
        //MODO INVITADO
            ...
            if (parse_comandos[0] == "/start") {
                ...
                bot.sendMessage(String(ID_TELEGRAM),
                        "User: " + chat_id + " @" + bot.messages[i].username + " dice: " + text, "");
        }
    }
}
